...
def sweeping_window(data, window, si, stepping=1, manual_idx=None, use_fftw=False, numcore=4, mc=True, noiselimit=None):
    ...
    if not manual_idx:
        ...
        for index in range(len(data)):
            if (index >= len(window)/2) and (index < len(data)-(len(window)/2)):
                if (index % stepping==0):
                    fid_temp = np.zeros(si)*1.0j
                    fid_temp[int(index-len(window)/2):int(index+len(window)/2)] = np.multiply(data[int(index-len(window)/2):int(index+len(window)/2)], window)
                    if use_fftw:
        ...
        fid_temp = np.zeros(len(data))*1.0j
        fid_temp[int(index-len(window)/2):int(index+len(window)/2)] = np.multiply(data[int(index-len(window)/2):int(index+len(window)/2)], window)
